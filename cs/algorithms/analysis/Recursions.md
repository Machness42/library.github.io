...
**קריאה נדרשת: [סימונים אסימפטוטים](https://machness42.github.io/library/cs/algorithms/analysis/Asymptotic Notations)**, אינדוקציה, עצים
...
מצאו חסם אסימפטוטי הדוק ככל הניתן על $\text{fibb}(n)$.
...
$$
f(n) = 2^{n-1} = \frac{1}{2}\cdot2^n \in O\left(2^n\right)
$$

כפי שרצינו.

כמובן, לא כל המקרים יהיו ככה. זה היה מקרה מאוד פשוט אבל לפעמים הוא עובד. דרך שאמנם פחות פורמלית אך יותר אינטואיטיבית להבנה היא בעזרת עץ רקורסיה.

![A recursion tree for $f$](imgs/RecTree.png)

אם תשימו לב, ניתן לראות שכל קריאה של $f$ מתחלקת לשתי קריאות ואפשר ליצור "עץ" של הקריאות. סך כל הערכים הסופיים (תנאי העצירה) נמצאים בעלים. כיוון שזהו עץ מושלם, וגובה העץ הוא $h=n-1$ אז יש $2^h=2^{n-1}$ עלים אך זהו ערך הפונקציה!